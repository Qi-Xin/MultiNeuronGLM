"""Exp 2c: model-variant robustness of the coupling filters, using the EXACT main-results
6-area pop-GLM (time-warped baseline + trial_coef).

Models (all: baseline num=20, coupling from all 6 probes, trial_coef, warped, penalty=5e-1):
  main    : selected populations (group_id==0) + refractory_additive (f_damp)
  fullpop : ALL neurons per probe (pooling use_all=True) + refractory_additive      [S6]
  linhist : selected populations, NO refractory_additive -> linear self-history only [S7]
Fitted for every target probe and both conditions -> all 36 coupling filters per model/condition.
"""
...
OUT="/sessions/gifted-cool-babbage/mnt/outputs/exp2c_variants.pkl"
...
def fit_one(model, cond, target):
    ...
    # coupling effects are indices 1..6 (probeA..probeF)
    return {PROBES[j]: np.asarray(m.get_filter()[1+j]).tolist() for j in range(6)}
...
def main():
    ...
    for key in todo:
        ...
        print("fitted %-8s %-10s target=%-3s (%.0fs)  [%d/36]"
              %(key[0],key[1],AREA[key[2]],time.time()-t,len(R)),flush=True)
