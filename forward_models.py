...
def fwd_model_1d(gt_csd , z, x, R):
    if gt_csd.ndim == 2:
        lfp = fwd_model_1d_single (gt_csd , z, x, R)
    elif gt_csd.ndim == 3:
        lfp = np.zeros((len(x), gt_csd.shape[1], gt_csd.shape[2]))
        for itrial in range(gt_csd.shape[2]):
            lfp[:, :, itrial] = fwd_model_1d_single (gt_csd[:, :, itrial] , z, x, R)
    else:
        raise ValueError("Ground true CSD dimension error! ")
    return lfp

def fwd_model_1d_single (csd , z, x, R):
    """
    ...
    lfp = np. zeros ((nx , nt))
    for t in range (nt):
        csd_tmp = np. squeeze ( csd [:, t]) [:, None ]
        A = b_fwd_1d(z.T-x, R)
        lfp[:, t] = (A@csd_tmp).squeeze()
    return lfp
